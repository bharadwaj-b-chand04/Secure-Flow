...
export class DatabaseService {
  ...
  async initializeDatabase(): Promise<void> {
    try {
      ...
      this.db = await openDatabaseAsync('secureflow.db'); // ✅ Async modular API
  ...
  private async createTables(): Promise<void> {
    ...
    const createBehaviorProfilesTable = `
    ...
    await this.db.execAsync(createSensorLogsTable);
    await this.db.execAsync(createBehaviorProfilesTable);
    await this.db.execAsync(createSecurityEventsTable);
  ...
  async clearOldSensorData(olderThanDays: number = 30): Promise<void> {
    ...
    try {
      ...
      console.log(`✅ Cleared ${result.changes} sensor records older than ${olderThanDays} days`);
    } catch (error) {
  ...
  async saveBehaviorProfile(userId: string, profile: BehaviorProfileData): Promise<void> {
    ...
    try {
      ...
      console.log('✅ Behavior profile saved successfully');
    } catch (error) {
  ...
  async getDatabaseStats(): Promise<{
    ...
    try {
      const sensorCount = await this.db.getFirstAsync('SELECT COUNT(*) as count FROM sensor_logs') as any;
      const profileCount = await this.db.getFirstAsync('SELECT COUNT(*) as count FROM behavior_profiles') as any;
      const eventCount = await this.db.getFirstAsync('SELECT COUNT(*) as count FROM security_events') as any;

      return {
        sensorLogs: sensorCount.count || 0,
        behaviorProfiles: profileCount.count || 0,
        securityEvents: eventCount.count || 0,
        databaseSize: 'N/A'
      };
    } catch (error) {
